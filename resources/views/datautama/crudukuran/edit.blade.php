<x-layout>
    ...
    <h1 class="pb-2 mb-4 text-2xl font-semibold text-gray-800 border-b-2 border-gray-300">
        <span class="text-blue-600">Edit Ukuran</span>
    </h1>

    <form action="{{ route('ukuran.update', $ukuran->id_ukuran) }}" method="POST" enctype="multipart/form-data"
        class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <!-- Nama Kategori -->
            <div>
                <label for="ukuran" class="block text-sm font-medium text-gray-700">Nama Ukuran:</label>
                <input type="text" name="ukuran" id="ukuran" value="{{ old('ukuran', $ukuran->ukuran) }}"
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('ukuran') border-red-500 @enderror"
                    required>
                @error('ukuran')
