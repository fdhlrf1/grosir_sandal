<!-- resources/views/datautama/d_motif.blade.php -->

<x-layout>
    <x-slot:title>{{ $title }}</x-slot:title>

    <h2
        class="flex items-center p-4 mb-6 space-x-3 text-3xl font-bold text-gray-800 rounded-lg shadow-md bg-gradient-to-r from-fuchsia-200 to-fuchsia-400">
        <i class="text-4xl text-fuchsia-600 fas fa-border-all"></i>
        <!-- Mengganti ikon menjadi ikon warna -->
        <span class="flex flex-col">
            <span>Data Motif</span>
            <span class="mt-1 text-lg font-normal text-gray-700">Manajemen dan Pengelolaan Motif Barang Anda</span>
        </span>
    </h2>

    <div class="flex items-center justify-between mb-4">
        <!-- Form Pencarian -->
        <form action="{{ route('motif.index') }}" method="GET" class="flex items-center flex-grow">
            <input type="text" name="search" placeholder="Cari motif..." value="{{ request()->get('search') }}"
        ...
        @include('datautama.crudmotif.modal-create')
        @include('datautama.crudmotif.modal-edit')
        @include('datautama.crudmotif.modal-delete')

        <!-- Modal toggle -->
        @if (Auth::User()->role->nama_role === 'Admin')
            <button data-modal-target="tambah-motif-modal" data-modal-toggle="tambah-motif-modal"
        ...
        <table class="w-full overflow-hidden border-collapse rounded-lg shadow-lg table-auto">
            <thead class="text-xs font-medium tracking-wider text-left text-gray-600 uppercase bg-gray-200">
                <tr>
                    ...
                    <th class="px-3 py-3 border border-slate-300">Nama motif</th>
                ...
                @forelse ($motifs as $motif)
                    <tr class="transition-colors duration-300 ease-in-out hover:bg-gray-100">
                        <td class="px-3 py-4 border border-slate-300 whitespace-nowrap">
                            {{ $loop->iteration + $motifs->firstItem() - 1 }}</td>
                        <td class="px-3 py-4 border border-slate-300 whitespace-nowrap">{{ $motif->nama_motif }}
                        </td>
                        ...
                        @if (Auth::User()->role->nama_role === 'Admin')
                            <td class="px-2 py-4 border border-slate-300 whitespace-nowrap">
                                <div class="flex justify-center space-x-2">
                                    <button data-modal-target="edit-motif-modal{{ $motif->id_motif }}"
                                        data-modal-toggle="edit-motif-modal{{ $motif->id_motif }}"
                                    ...
                                    <button data-modal-target="hapus-motif-modal{{ $motif->id_motif }}"
                                        data-modal-toggle="hapus-motif-modal{{ $motif->id_motif }}"
