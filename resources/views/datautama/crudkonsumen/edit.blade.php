<x-layout>
    ...
    <h1 class="pb-2 mb-4 text-2xl font-semibold text-gray-800 border-b-2 border-gray-300">
        <span class="text-blue-600">Edit Konsumen</span>
    </h1>

    <form action="{{ route('konsumen.update', $konsumen->id_konsumen) }}" method="POST" enctype="multipart/form-data"
        class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <!-- Nama Konsumen -->
            <div>
                <label for="nama" class="block text-sm font-medium text-gray-700">Nama Konsumen:</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama', $konsumen->nama) }}"
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="4"
                    ...
                    required>{{ old('alamat', $konsumen->alamat) }}</textarea>
                @error('alamat')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
